...
function getRandomPos() {
  const margin = 80;
  const hudH = 90;
  const w = window.innerWidth - margin * 2 - 140;
  const h = window.innerHeight - hudH - 60 - margin;
  return {
    x: margin + Math.random() * w,
    y: hudH + margin + Math.random() * h,
  };
}
...
function addPairToBoard(pair) {
  ...
  pair.status = 'active'; // active | matched | wrong
...
function initBoard() {
  ...
  initial.forEach(p => addPairToBoard(p));
}
